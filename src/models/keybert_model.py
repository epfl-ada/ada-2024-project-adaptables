...
# Ever so slightly modified version of Keybert class to support GPU extraction
# All credits for the initial implemnetationgo to Maarten Grootendorst
...
class KeyBERT:
    ...
    def extract_keywords(
        ...
        use_tqdm: bool = False,
        model_encode_kwargs : dict|None = None,
        ...
        if doc_embeddings is None:
            doc_embeddings = self.model.embed(docs,verbose=use_tqdm,extra_kwargs=model_encode_kwargs)
        if word_embeddings is None:
            word_embeddings = self.model.embed(words,verbose=use_tqdm,extra_kwargs=model_encode_kwargs)
        ...
        if seed_keywords is not None:
            if isinstance(seed_keywords[0], str):
                ...
                seed_embeddings = np.vstack(
                    [
                        self.model.embed(keywords,extra_kwargs=model_encode_kwargs.update({"device":"cpu"})).mean(axis=0, keepdims=True) if len(keywords) != 0 else doc_embeddings[i][None,:]
                        for i,keywords in (tqdm(enumerate(seed_keywords),desc="Embedding seed keywords",total=len(seed_keywords)) if use_tqdm else enumerate(seed_keywords))
                    ]
                )
        ...
        # Find keywords
        all_keywords = []
        for index, _ in tqdm(enumerate(docs),total=len(docs),desc="Finding keywords:") if use_tqdm else enumerate(docs):
